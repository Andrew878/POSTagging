...
class BackwardProbabilities:
    ...
    def final_return_prob_sum(self, exisiting_alpha, previous_word_index, first_word):

        prob_est = 0

        for previous_state_index in range(len(self.non_beg_or_end_states)):

            prob_current_word_given_state = log(self.smoothed_word_tag[self.non_beg_or_end_states[previous_state_index]].prob(first_word))
            ...
            prob_est += exp(exisiting_alpha[previous_word_index][previous_state_index] + log(
                self.smoothed_tag_tag[self.start].prob(self.non_beg_or_end_states[previous_state_index]))+prob_current_word_given_state)
    ...
    def return_backward_prob(self, word_list):
        ...
        total_path_prob = self.final_return_prob_sum(alpha, word_num - 1, word_list[word_num])
